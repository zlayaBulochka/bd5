...
class Main(tk.Frame):
    ...
    def init_main(self):
        ...
        self.tree = ttk.Treeview(self, columns=( 'ID', 'surname', 'nam', 'patronymic', 'citizenship', 'date_of_birth', 'place_of_birth', 'sex', 'profession',
    ...
    def edit_record(self, surname, nam,  patronymic, citizenship, date_of_birth, place_of_birth, sex, profession, phone):
        self.db.c.execute('''UPDATE finance SET surname=?, nam=?, patronymic=?, citizenship=?, date_of_birth=?, place_of_birth=?, sex=?, profession=?, phone=? WHERE ID=?''',
    ...
    def search_records(self, surname):
        surname = ('%' + surname + '%',)
        self.db.c.execute('''SELECT * FROM finance WHERE surname LIKE ? ''', surname)
        [self.tree.delete(i) for i in self.tree.get_children()]
        [self.tree.insert('', 'end', values=row) for row in self.db.c.fetchall()]
...
class DB:
    def __init__(self):
        ...
        self.c.execute(
            '''CREATE TABLE IF NOT EXISTS finance (id integer primary key, surname varchar, nam varchar,  patronymic varchar, citizenship varchar, date_of_birth date, place_of_birth varchar, sex varchar, profession varchar, phone integer) ''')
        self.conn.commit()

    def insert_data(self, surname, nam, patronymic, citizenship, date_of_birth, place_of_birth, sex, profession, phone):
        self.c.execute('''INSERT INTO finance(surname, nam, patronymic, citizenship, date_of_birth, place_of_birth, sex, profession, phone) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)''',
...
if __name__ == "__main__":
    ...
    app = Main(root)
    app.pack()
    root.title("Иностранные граждане")
